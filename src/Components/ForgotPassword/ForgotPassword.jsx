  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData || 'Email address not found.');
      }
    } catch (err) {
  
      setError('Connection failed. Please check your server status or CORS settings.');
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center py-12 px-4">
      <div className="max-w-md w-full bg-white p-10 rounded-[2.5rem] shadow-2xl border border-gray-100">
        ...
        {message && (
          <div className="mb-4 p-3 bg-green-50 text-green-700 rounded-xl text-center text-sm font-bold border border-green-100">
            {message}
          </div>
        )}
        
        {error && (
          <div className="mb-4 p-3 bg-red-50 text-red-700 rounded-xl text-center text-sm font-bold border border-red-100">
            {error}
          </div>
        )}
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <button 
            type="submit" 
            disabled={loading} 
            className="w-full py-4 rounded-2xl shadow-lg font-bold text-white bg-green-600 hover:bg-green-700 transition-all disabled:opacity-50"
          >
